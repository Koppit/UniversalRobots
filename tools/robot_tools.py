class RobotActionTools:
    ...
    z_height_pick_mm  =  10   # 1 cm over bordet (plukkhøyde)
    ...
    def _move(self, label: str, rx: float, ry: float, z_mm: float):
        ...
        ry_rel = -ry
        z_rel  = -z_mm / 1000
        coords = [rx, ry_rel, z_rel, *self._ROTATION]
        self._log("info",
            f"  {label}: X={rx*1000:+.1f}mm Y={ry*1000:+.1f}mm Z=+{z_mm:.1f}mm (over bordet)"
            f"  rx={self._ROTATION[0]:.3f} ry={self._ROTATION[1]:.3f} rz={self._ROTATION[2]:.3f}")
    ...
    def pick_object_at(self, normalized_y: int, normalized_x: int):
        ...
        self._move("3/5 ned       ", rx, ry, self.z_height_pick_mm)
        self._log("info", "  4/5 klem griper")
        self.robot.grab_object()
        self._move("5/5 opp       ", rx, ry, self.z_height_hover_mm)
    ...
    def place_object_at(self, normalized_y: int, normalized_x: int):
        ...
        self._lift_to_hover()
        self._move("1/5 hover     ", rx, ry, self.z_height_hover_mm)
        self._move("2/5 ned       ", rx, ry, self.z_height_pick_mm)
